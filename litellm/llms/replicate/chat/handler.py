...
def completion(
    ...
    if "stream" in optional_params and optional_params["stream"] is True:
        ...
        for retry in range(litellm.DEFAULT_REPLICATE_POLLING_RETRIES):
            ...
            )  # wait to allow response to be generated by replicate - else partial output is generated with non-final status
            response = httpx_client.get(url=prediction_url, headers=headers)
            if response.status_code == 200 and response.json().get("status") not in [
                "succeeded",
                "failed",
                "canceled",
            ]:
